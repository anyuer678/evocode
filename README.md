...
> **安全边界说明**：本项目**无认证机制**，仅监听 `127.0.0.1`（本地回环）。Analyzer 与 Backend 之间仅允许本机通信。CI 排除了需要真实 PostgreSQL/Redis 的集成测试（`EvocodeApplicationTests`）。请勿在不受信任的网络环境中暴露端口。
...
**安全声明**：本项目**无认证/鉴权机制**，默认仅监听 `127.0.0.1`（本地回环），依赖操作系统网络隔离。Analyzer API 仅接受本机请求。CI 已排除需要真实数据库的集成测试。若需在多机/公网环境使用，请自行添加认证层。
