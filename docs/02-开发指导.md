...
scripts/deploy.sh:
  ...
  2. 安装 Python 依赖，systemd 托管 analyzer（127.0.0.1:8081）
...
| analyzer | 8081 | **仅 127.0.0.1** | 内部分析服务（/analyze/v1） |
...
3. 起 analyzer：uvicorn app.main:app --port 8081（探活 /health）
...
```bash
curl http://127.0.0.1:8081/health          # analyzer: {"status":"ok"}
